  ...
  const int sensor_index, Projection & projection) const
{
  ...
  if (!std::isfinite(range) || range < sensor.min_range) {
    return false;
  }

  // Exact 1.1.1 spherical projection: local Z is the sensor forward axis,
  // azimuth wraps around it, and elevation is the polar angle from it.
  const Eigen::Vector3f local = sensor.rotation_body_sensor.transpose() * delta;
  const float forward_score = local.z() / range;
  if (forward_score <= 0.0F) {
    return false;
  }

  float azimuth_deg = std::atan2(local.y(), local.x()) * kRadToDeg;
  if (azimuth_deg < 0.0F) {
    azimuth_deg += 360.0F;
  }
  const float elevation_deg =
    std::acos(std::clamp(forward_score, -1.0F, 1.0F)) * kRadToDeg;

  const Eigen::Vector3f forward = sensor.rotation_body_sensor.col(2).normalized();
  const float forward_distance = delta.dot(forward);
  bool in_base_fov = false;
  if (sensor.use_frustum) {
    const float radial_distance = std::sqrt(std::max(
      0.0F, range * range - forward_distance * forward_distance));
    const float apex_shift = (sensor.origin - sensor.frustum_apex).dot(forward);
    const float shifted_forward = forward_distance + apex_shift;
    if (shifted_forward > 0.0F) {
      const float min_radius = shifted_forward * std::tan(sensor.fov_min_deg / kRadToDeg);
      const float max_radius = shifted_forward * std::tan(sensor.fov_max_deg / kRadToDeg);
      in_base_fov = radial_distance >= min_radius && radial_distance <= max_radius;
    }
  } else {
    in_base_fov = elevation_deg >= sensor.fov_min_deg &&
      elevation_deg <= sensor.fov_max_deg;
  }

  bool in_extra_coverage = false;
  if (!in_base_fov && sensor.extra_coverage_enabled && range <= sensor.extra_max_range) {
    Eigen::Vector3f left = Eigen::Vector3f::UnitZ().cross(forward);
    const float left_norm = left.norm();
    if (left_norm > kEpsilon && forward_distance > 0.0F) {
      left /= left_norm;
      const float yaw_deg = std::atan2(delta.dot(left), forward_distance) * kRadToDeg;
      const float pitch_deg = std::atan2(
        delta.z(), std::max(forward_distance, kEpsilon)) * kRadToDeg;
      in_extra_coverage = yaw_deg >= sensor.extra_yaw_min_deg &&
        yaw_deg <= sensor.extra_yaw_max_deg &&
        pitch_deg >= sensor.extra_pitch_min_deg &&
        pitch_deg <= sensor.extra_pitch_max_deg;
    }
  }
  if (!in_base_fov && !in_extra_coverage) {
    return false;
  }

  const float elevation_normalized =
    (elevation_deg - sensor.effective_elevation_min_deg) /
    std::max(kEpsilon,
      sensor.effective_elevation_max_deg - sensor.effective_elevation_min_deg);

  projection.sensor_index = sensor_index;
  projection.azimuth_index = wrapAzimuth(static_cast<int>(
    azimuth_deg * static_cast<float>(config_.azimuth_bins) / 360.0F));
  ...
  projection.forward_score = forward_score;
  return true;
}
  ...
  Projection & projection) const
{
  ...
  for (std::size_t index = 0; index < sensors.size(); ++index) {
    ...
    if (!found || candidate.forward_score > best.forward_score) {
      best = candidate;
      found = true;
    }
  }
...
void EvidenceMap::transformAndReindex(const Eigen::Isometry3f & current_from_previous)
{
  ...
  for (const auto & item : voxels_) {
    ...
    if (!inserted) {
      ...
      const uint32_t target_hits = std::max<uint32_t>(1U, target.hit_count);
      const uint32_t source_hits = std::max<uint32_t>(1U, voxel.hit_count);
      const uint32_t combined_hits = target_hits + source_hits;
      target.position =
        (target.position * static_cast<float>(target_hits) +
        voxel.position * static_cast<float>(source_hits)) /
        static_cast<float>(combined_hits);
      target.hit_count = static_cast<uint16_t>(std::min<uint32_t>(255U, combined_hits));
      target.occupancy_score = std::min(
        config_.score_cap, target.occupancy_score + 0.5F * voxel.occupancy_score);
      target.free_score = std::min(
        config_.score_cap, std::max(target.free_score, voxel.free_score));
  ...
  const std::vector<SensorModel> & sensors) const
{
  ...
  // Vendor 1.1.1 protects bins around actual returns. This protection only
  // suppresses weak clearing on missing returns; explicit free-space evidence
  // in front of a measured return remains authoritative.
  const int protect_radius = std::max(0, config_.visibility_protect_radius);
  for (auto & map : maps) {
    std::vector<uint8_t> protected_bins(bin_count, 0U);
    for (int elevation = 0; elevation < config_.elevation_bins; ++elevation) {
      for (int azimuth = 0; azimuth < config_.azimuth_bins; ++azimuth) {
        const VisibilityBin & source = map.bins[visibilityIndex(azimuth, elevation)];
        if (source.hit_count == 0U) {
          continue;
        }
        for (int de = -protect_radius; de <= protect_radius; ++de) {
          const int protected_elevation = elevation + de;
          if (protected_elevation < 0 || protected_elevation >= config_.elevation_bins) {
            continue;
          }
          for (int da = -protect_radius; da <= protect_radius; ++da) {
            const int protected_azimuth = wrapAzimuth(azimuth + da);
            protected_bins[visibilityIndex(protected_azimuth, protected_elevation)] = 1U;
          }
        }
      }
    }
    for (std::size_t index = 0; index < bin_count; ++index) {
      map.bins[index].protected_bin = protected_bins[index] != 0U;
    }
  }
  return maps;
}
  ...
  ProcessStatistics & statistics)
{
  for (auto iterator = voxels_.begin(); iterator != voxels_.end();) {
    ...
    {
      voxel.consecutive_free_frames = 0U;
      if (projection.range > config_.near_field_protect_range) {
    ...
    {
      if (voxel.consecutive_free_frames != std::numeric_limits<uint32_t>::max()) {
        ++voxel.consecutive_free_frames;
      }
  ...
  ProcessStatistics & statistics)
{
  ...
  for (const auto & item : accumulators) {
    ...
    if (inserted || voxel.hit_count == 0U) {
      ...
      const uint32_t old_weight = std::max<uint32_t>(1U, voxel.hit_count);
    ...
    voxel.hit_count = static_cast<uint16_t>(std::min<uint32_t>(
      255U, static_cast<uint32_t>(voxel.hit_count) + accumulator.count));
